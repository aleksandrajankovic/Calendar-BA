...
export default function CalendarMobileFootball({
  ...
  const [swipeHintPhase, setSwipeHintPhase] = useState("visible");
  ...
  const ballRing = isGold
    ...
    : isToday
    ? "ring-[4px] ring-[#FACC01] shadow-[0_0_40px_rgba(250,204,1,1)]"
    ...
    <div className="w-full flex flex-col items-center justify-start pt-3 pb-8 gap-5 [--football-ball-size:min(72vw,320px)] [@media(min-height:800px)]:gap-6 [@media(min-height:800px)]:[--football-ball-size:min(68vw,292px)]">
      ...
      <div className="w-full flex flex-col">
            ...
            <div className="flex h-full items-center gap-2.5 w-max">
              {days.map((day, index) => {
                ...
                return (
                  ...
                  >
                    <span
                      className={`absolute inset-0 rounded-sm ${isActive ? (isDayToday ? "bg-[#FFD700]" : "bg-white") : "bg-transparent"}`}
                      style={{
                    ...
                    {isDayPromo && (
                      <span className="absolute bottom-1 left-1/2 z-20" style={{ transform: "translateX(-50%)" }}>
                        <span className="block rounded-full promo-dot-pulse" style={{ width: 5, height: 5, background: "#4CAF50" }} />
                      </span>
                    )}

                    <span
                      className={`relative z-10 ${russoOne.className} text-[18px] leading-none italic font-bold ${isActive ? "text-black" : "text-white"}`}
                    >
                      {day.day}
                    </span>
                  </button>
        ...
        <div className="flex justify-center items-center gap-1.5 mt-2.5 px-4">
          <svg
            ...
            style={{ opacity: swipeHintPhase === "hidden" ? 0 : swipeHintPhase === "fading" ? 0 : 1, transition: "opacity 0.5s" }}
          >
      ...
      <div
        ...
        onTouchEnd={(e) => {
          ...
          if (Math.abs(dx) > 40 && Math.abs(dx) > Math.abs(dy)) {
            ...
            if (dx < 0) setSelectedIndex((i) => Math.min(i + 1, days.length - 1));
      ...
      >
        <div className="relative flex items-center justify-center">
          ...
          {isToday && (
            <span
              className="pointer-events-none absolute rounded-full z-20 today-ring-pulse"
              style={{
                width: "var(--football-ball-size)", height: "var(--football-ball-size)",
                border: "4px solid #FACC01",
              }}
            />
          )}
          ...
          <button
            ...
            className={`
              ...
              w-(--football-ball-size) h-(--football-ball-size)
              ${ballRing}
