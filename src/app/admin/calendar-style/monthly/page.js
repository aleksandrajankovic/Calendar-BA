...
export default function MonthlySettingsPage() {
  ...
  const configuredKeys = useMemo(
    () => Object.keys(monthBackgrounds).filter((k) => {
      ...
      return v?.desktop || v?.mobile || v?.position || v?.titleBa || v?.inactive || v?.theme;
    }),
    [monthBackgrounds]
  );
                ...
                <div className="space-y-3">
                  <div>
                    <input
                      ...
                      value={currentBg.titleBa || ""}
                      onChange={(e) => setField("titleBa", e.target.value)}
                      placeholder="Leave empty to use global title"
                    />
                  </div>
                  {currentBg.titleBa && (
                    <p className="text-xs text-neutral-400">
                      Preview: <span className="font-bold text-neutral-700">{currentBg.titleBa}</span>
                    </p>
                  )}
                </div>
              </div>
