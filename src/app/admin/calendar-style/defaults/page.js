...
export default function DefaultSettingsPage() {
  ...
  const [titleBa, setTitleBa]                   = useState("KALENDAR PROMOCIJA");
  const [logoUrl, setLogoUrl]                   = useState("/img/meridianbet-ng.png");
  const [theme, setTheme]                       = useState("default");
  const [seoTitleBa, setSeoTitleBa]             = useState("");
  const [seoDescBa, setSeoDescBa]               = useState("");
  ...
  useEffect(() => {
    fetch("/api/calendar-settings")
      ...
      .then((d) => {
        ...
        setTitleBa(d.calendarTitle?.ba || "KALENDAR PROMOCIJA");
        setLogoUrl(d.logoUrl || "/img/meridianbet-ng.png");
        setTheme(d.theme || "default");
        setSeoTitleBa(d.seoMeta?.ba?.title || "");
        setSeoDescBa(d.seoMeta?.ba?.description || "");
      })
    ...
    try {
      const res = await fetch("/api/calendar-settings", {
        ...
        body: JSON.stringify({
          ...
          calendarPosition,
          calendarTitle: { ba: titleBa },
          logoUrl,
          theme,
          seoMeta: { ba: { title: seoTitleBa, description: seoDescBa } },
        }),
      });
      ...
      {loading ? (
        ...
        <div className="space-y-6">
            ...
            <div className="p-4 space-y-4">
              ...
              <div>
                <input
                  ...
                  value={titleBa}
                  onChange={(e) => setTitleBa(e.target.value)}
                  placeholder="KALENDAR PROMOCIJA"
                />
              </div>
              {titleBa && (
                <div className="pt-2 border-t border-neutral-100">
                  <p className="text-xs text-neutral-400 mb-1">Preview (BA):</p>
                  <p className="text-2xl font-extrabold tracking-tight text-neutral-800">{titleBa}</p>
                </div>
              )}
            </div>
          </AdminTableCard>
          ...
          <AdminTableCard title="Calendar Theme">
            <div className="p-4 space-y-4">
              <p className="text-sm text-neutral-500">
                Choose the visual style for the calendar.
              </p>
          ...
          <AdminTableCard title="SEO / Meta Tags">
            <div className="p-4 space-y-4">
              ...
              <label className="block">
                <span className="mb-1 inline-block text-sm text-neutral-600">Meta title (BA)</span>
                <input
                  className="w-full border border-[#D0D0D0] rounded px-2.5 py-2 text-sm"
                  value={seoTitleBa}
                  onChange={(e) => setSeoTitleBa(e.target.value)}
                  placeholder="Kalendar Promocija | Meridianbet"
                  maxLength={120}
                />
                <span className="mt-0.5 text-xs text-neutral-400">{seoTitleBa.length}/120 characters</span>
              </label>
              <label className="block">
                <span className="mb-1 inline-block text-sm text-neutral-600">Meta description (BA)</span>
                <textarea
                  className="w-full border border-[#D0D0D0] rounded px-2.5 py-2 text-sm resize-none"
                  rows={3}
                  value={seoDescBa}
                  onChange={(e) => setSeoDescBa(e.target.value)}
                  placeholder="Otkrijte dnevne promocije i iskoristite ekskluzivne nagrade..."
                  maxLength={320}
                />
                <span className="mt-0.5 text-xs text-neutral-400">{seoDescBa.length}/320 characters</span>
              </label>
            </div>
          </AdminTableCard>
